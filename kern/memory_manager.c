...
void * create_page_table(uint32 *ptr_page_directory, const uint32 virtual_address)
{
	...
	void* Page_Table=kmalloc(PAGE_SIZE);
	memset(Page_Table,0,PAGE_SIZE);
	ptr_page_directory[PDX(virtual_address)]=kheap_physical_address((uint32)Page_Table)|PERM_WRITEABLE|PERM_PRESENT|PERM_USER;
	tlbflush();
	return Page_Table;
}
...
//	unmap_frame(e->env_page_directory,(void*)va);
	for(int i=0; i<WsSize; i++) //2
	{
		va=e->ptr_pageWorkingSet[i].virtual_address;
     if(va>=virtual_address && va<end )
     {
	...
	for(;va<end; va+=PAGE_SIZE)
	{
		...
		if(ptr_page_table!=NULL && ptr_page_table!=prev_page_table)
		{

		cnt=0;
			for(int i=0; i<1024; i++)
				{
					if((ptr_page_table[i]&PERM_PRESENT)>0)
					{
						cnt++;
					}
				}
			...
			prev_page_table=ptr_page_table;
		}
	}
...
{
	//TODO: [PROJECT 2018 - BONUS3] User Heap Realloc [Kernel Side]
	//your code is here, remove the panic and write your code
