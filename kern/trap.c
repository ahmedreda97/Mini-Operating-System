...
void Remove_Using_LRU(struct Env * curenv, uint32 fault_va,int type);
...
void page_fault_handler(struct Env * curenv, uint32 fault_va)
{
	...
	//cprintf("fault va = %x\n", fault_va);
	/* =============================================== CASE I [MILESTONE II] ==============================================*/
	/* When memory becomes SCARCE, removing less recently used pages out of the working sets of active processes,
	 * reducing the size of those resident sets*/


	uint32 Number_Of_Free_Frames=calculate_free_frames()*100;
	Number_Of_Free_Frames/=number_of_frames;
//	Number_Of_Free_Frames*=100;

//	cprintf("HEREEEEEEEEEEEEEEEEE %d  %d\n",x++,Number_Of_Free_Frames);
	if(Number_Of_Free_Frames<memory_scarce_threshold_percentage){

		Remove_Using_LRU(curenv,fault_va,0);

		for(int i=0;i<num_of_ready_queues;i++)
		{
			struct Env_Queue * ptr_env_queue = &env_ready_queues[i];
			struct Env * ptr_env=NULL;
			if (ptr_env_queue == NULL)
			{
				continue;
			}
			LIST_FOREACH(ptr_env,ptr_env_queue)
			{
				if(ptr_env==NULL)
				{
					continue;
				}
				Remove_Using_LRU(ptr_env,fault_va,0);
		...
		LIST_FOREACH(ptr_env,&env_exit_queue)
		{
			...
			Remove_Using_LRU(ptr_env,fault_va,1);
		}
	...
	/* When WS if FULL, removing less recently used pages out of the working set of the CURRENT large process that
	...
	Placement(curenv,fault_va);
			...
			//env_page_ws_clear_entry()

		//TODO: [PROJECT 2018 - BONUS5] Change WS Size according to “Program Priority”

}
// if type ==0 it is ready queue or case II else it is exit queue
void Remove_Using_LRU(struct Env * curenv, uint32 fault_va,int type){

	uint32 Pages_To_Free=((curenv->percentage_of_WS_pages_to_be_removed
				*env_page_ws_get_size(curenv))+99)/100;
	if(type==1){
		Pages_To_Free=curenv->page_WS_max_size;
	}
	for(int i=0;i<Pages_To_Free;i++){
		uint32 victim=-1;

		for(int j=0;j<curenv->page_WS_max_size;j++){
			if(curenv->ptr_pageWorkingSet[j].empty==0 ){
				if(victim==-1||env_page_ws_get_time_stamp(curenv,j)
						<env_page_ws_get_time_stamp(curenv,victim)){
					victim=j;
				}
			}
		}
		if(victim==-1){
			...
			uint32 va=env_page_ws_get_virtual_address(curenv,victim);
			uint32 perms=pt_get_page_permissions(curenv,va);
			uint32 modif=(perms&PERM_MODIFIED);
				uint32* ptr_pg_table=NULL;
			struct Frame_Info* frminfo=get_frame_info(curenv->env_page_directory,
					(void*)va,&ptr_pg_table);
			if(modif){
				pf_update_env_page(curenv,(void*)va,frminfo);
			}
...
void Placement(struct Env * curenv, uint32 fault_va){
	...
	uint32 * PPT=NULL ;
		    struct Frame_Info* Fi = get_frame_info(curenv->env_page_directory,(void*)fault_va,&PPT);
		    int r = allocate_frame(&Fi);
			if (r!=0) // couldn't find frame to allocate
			{
				return;
			}
			map_frame(curenv->env_page_directory,Fi,(void*)fault_va,PERM_USER|PERM_WRITEABLE|PERM_PRESENT);

			int read_error=pf_read_env_page(curenv,(void*)fault_va);
			if (read_error!=0 && fault_va>=USTACKBOTTOM && fault_va<=USTACKTOP)  //it's a stack page
			{
				pf_add_empty_env_page(curenv,fault_va,1);
			}
			//7etet el else if 3shan  run tia 15
