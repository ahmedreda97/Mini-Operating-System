...
struct UserProgramInfo userPrograms[] = {
		...
		{ "tef1", "", PTR_START_OF(tst_envfree1)},{ "ef_ms1", "", PTR_START_OF(ef_mergesort_noleakage)},
...
{
	//[PROJECT DONE - [2] Kernel Dynamic Allocation] create_user_page_WS()
...
{
	//[PROJECT DONE - [2] Kernel Dynamic Allocation] create_user_directory()
	// Write your code here, remove the panic and write your code
...
void env_free(struct Env *e)
{
	...
	//TODO: [PROJECT 2018 - BONUS4] Exit [env_free()]
...
//			cprintf("%u\n",va);
			...
			if(va>=USER_PAGES_WS_START && va<=USER_PAGES_WS_MAX){
			}
		}
	...
	for(int i=0;i<1024;i++){
		uint32 va=e->env_page_directory[i];
		va/=PAGE_SIZE;
		va*=PAGE_SIZE;
		if(va!=0)
//			kfree((void*)va);
//			cprintf("%u\n",va);
			freeMem(e,va,PAGE_SIZE);
		if ((va>=USER_PAGES_WS_START && va<=USER_PAGES_WS_MAX))
		{
		}
...
void __env_free_with_buffering(struct Env *e)
{
	...
	//[PROJECT 2015 - Exit] env_free()
	// your code is here, remove the panic and write your code
	panic("start_env_free() is not implemented yet...!!");
