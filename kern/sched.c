...
void sched_init_MLFQ(uint8 numOfLevels, uint8 *quantumOfEachLevel)
{
	...
	//=========================================
	//=========================================
	//TODO: [PROJECT 2018 - MS2 - [3] CPU Scheduling MLFQ] Initialize MLFQ
...
{
	//TODO: [PROJECT 2018 - MS2 - [3] CPU Scheduling MLFQ] MLFQ Scheduler
	// Write your code here, remove the panic and write your code
//	panic("fos_scheduler_MLFQ() is not implemented yet...!!");
	...
	uint32 Queue_index=-1;
//	cprintf("before setting \n");
//	sched_print_all();
	if(curenv!=NULL){
//		cprintf("setting curenv\n");
		enqueue(&(env_ready_queues[last]),curenv);
//		for(int i=last;i<num_of_ready_queues;i++){
//			if(curenv->nClocks<=quantums[i]){
//				enqueue(&(env_ready_queues[i]),curenv);
////				cprintf("cruenv id= %d at LEVEL= %d\n",curenv->env_id,i);
//				break;
//			}
//			else if(i==num_of_ready_queues-1){
//				enqueue(&(env_ready_queues[i]),curenv);
////				cprintf("cruenv id= %d at LEVEL= %d\n",curenv->env_id,i);
//			}
//		}
...
//	cprintf("getting next env\n");
	...
	for(int i=0;i<num_of_ready_queues;i++){
		...
		if(next_env!=NULL){
			...
			last=i+1;
			if(last==num_of_ready_queues){
				last--;
			}
			return next_env;
		}
//		struct Env*tmp=NULL;
//		LIST_FOREACH(tmp,&env_ready_queues[i]){
//			if(tmp!=NULL){
//				next_env=tmp;
//			}
//		}
...
//			cprintf("Next_env id= %d at LEVEL= %d\n",next_env->env_id,i);
//			return next_env;
//		}
	}
...
void sched_print_all()
{
	...
	if (!LIST_EMPTY(&env_new_queue))
	{
		...
		{
			cprintf("	[%d] %s CLOCK= %d\n", ptr_env->env_id, ptr_env->prog_name,ptr_env->nClocks);
		}
	}
	else
	{
